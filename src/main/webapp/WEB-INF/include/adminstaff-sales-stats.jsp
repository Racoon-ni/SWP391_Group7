    ...
    <body>
        <div class="container">
            ...
            <div class="export-form">
                ...
                <button type="button" onclick="exportExcel()">📤 Xuất báo cáo Excel</button>
            </div>
                    ...
                    <div class="value" id="thisMonthRevenueCard">
                        <c:choose>
                            <c:when test="${not empty thisMonthRevenue}">
                                <fmt:formatNumber value="${thisMonthRevenue}" pattern="#,##0.00" />₫
                            </c:when>
                            <c:otherwise>
                                ₫0.00
                            </c:otherwise>
                    ...
                    <tbody>
                        <c:forEach var="stat" items="${dailyStats}">
                            <tr>
                                ...
                                <td><fmt:formatNumber value="${stat.revenue}" pattern="#,##0.00" />₫</td>
                            </tr>
                        </c:forEach>
                        <c:if test="${empty dailyStats}">
                            <tr>
                                <td colspan="2" style="color:#777;">Không có dữ liệu theo ngày</td>
                            </tr>
                        </c:if>
                    </tbody>
                </table>
                ...
                <table id="monthlyTable">
                    ...
                    <tbody>
                        ...
                        <c:if test="${empty monthlyStats}">
                            <tr>
                                <td colspan="2" style="color:#777;">Không có dữ liệu theo tháng</td>
                            </tr>
                        </c:if>
                    </tbody>
                </table>
        ...
        <script>
                    ...
                    function buildSheetFromTable(tableId) {
                        ...
                        let rows = Array.from(table.querySelectorAll("tbody tr")).filter(tr => {
                        ...
                        aoa.push(["Tổng", total.toLocaleString('vi-VN', {minimumFractionDigits: 2}) + "₫"]);
                        const ws = XLSX.utils.aoa_to_sheet(aoa);
                        const colWidths = headers.map((h, i) => {
                            let max = h.length;
                            aoa.slice(1).forEach(row => {
                                if (row[i])
                                    max = Math.max(max, row[i].toString().length);
                            });
                            return {wch: max + 2};
                        });
                        ws['!cols'] = colWidths;
                    ...
                    function exportExcel() {
                        ...
                        const summaryData = [
                            ["BÁO CÁO DOANH THU"],
                            ["Tổng doanh thu", document.getElementById("totalRevenueCard")?.innerText || ""],
                            ["Hôm nay", document.getElementById("todayRevenueCard")?.innerText || ""],
                            ["Tháng này", document.getElementById("thisMonthRevenueCard")?.innerText || ""]
                        ];
                        ...
                        if (type === 'all' || type === 'day') {
                            ...
                            if (ws)
                                XLSX.utils.book_append_sheet(wb, ws, "Theo ngày");
                            if (type === 'all') {
                                const ws2 = buildSheetFromTable("monthlyTable");
                                if (ws2)
                                    XLSX.utils.book_append_sheet(wb, ws2, "Theo tháng");
                            }
                        } else if (type === 'month') {
                            ws = buildSheetFromTable("monthlyTable");
                            if (ws)
                                XLSX.utils.book_append_sheet(wb, ws, "Theo tháng");
                        }
                        if (!ws) {
