 ...
 * @author Admin
 */
@WebServlet(name = "StaffDetailServlet", urlPatterns = {"/StaffDetail"})
public class StaffDetailServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        // Chỉ cho xem nếu là staff
        if (staff == null || !"staff".equalsIgnoreCase(staff.getRole())) {
            response.sendError(404, "Không tìm thấy nhân viên.");
            return;
        }
        request.setAttribute("staff", staff);
        request.getRequestDispatcher("/WEB-INF/include/staff-detail.jsp").forward(request, response);
    }
